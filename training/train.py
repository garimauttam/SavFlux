"""
train.py — QLoRA fine-tuning for SavFlux using TRL + PEFT.

PURPOSE
-------
Fine-tuning teaches the model *behaviour and output format* — how SavFlux
...
Each line is a JSON object with a "messages" list in ChatML format:
  {"messages": [
      {"role": "system",  "content": "You are SavFlux..."},
...
----------
  ...
  # Then set OLLAMA_CHAT_MODEL=savflux-7b in your .env
...
def _write_ollama_modelfile(cfg: dict, merged_path: str) -> None:
    ...
    After writing this file:
      ...
      3. Register with Ollama:
           ollama create savflux-7b -f training/output/Modelfile
      4. Set OLLAMA_CHAT_MODEL=savflux-7b in .env and restart SavFlux.
    """
    modelfile_path = Path(cfg["output_dir"]).parent / "Modelfile"
    model_name = Path(cfg["model_name"]).name.lower().replace(".", "-")

    modelfile_content = (
        f"# Ollama Modelfile — SavFlux fine-tuned {model_name}\n"
        f"# Generated by training/train.py\n\n"
        f"FROM ./merged\n\n"
        f'SYSTEM """You are SavFlux, a grounded AI code reviewer. '
    ...
    print(
        ...
        f"       python llama.cpp/convert_hf_to_gguf.py {merged_path} --outfile savflux.gguf\n"
        "  2. (Optional) quantise for CPU/low-VRAM:\n"
        "       llama.cpp/llama-quantize savflux.gguf savflux-q4_k_m.gguf Q4_K_M\n"
        "  3. Register with Ollama:\n"
        f"       ollama create savflux-7b -f {modelfile_path}\n"
        "  4. Update .env:\n"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="QLoRA fine-tuning for SavFlux",
